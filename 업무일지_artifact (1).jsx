...
const TODAY_KEY = DAY_KEYS[Math.min(new Date().getDay() - 1, 4)] || "mon";
const DEFAULT_TASK_TEXT = "치료비 지원 상담";
...
const initialDayData = () => ({
  tasks: [{ id: Math.random(), text: DEFAULT_TASK_TEXT, done: false, status: "예정" }],
  special: "",
});
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const ensured = {};
        DAY_KEYS.forEach((k) => {
          const day = parsed[k] || { tasks: [], special: "" };
          // 기본 업무가 없으면 맨 앞에 추가
          const hasDefault = day.tasks.some((t) => t.text === DEFAULT_TASK_TEXT);
          const tasks = hasDefault
            ? day.tasks
            : [{ id: Math.random(), text: DEFAULT_TASK_TEXT, done: false, status: "예정" }, ...day.tasks];
          ensured[k] = { ...day, tasks };
        });
        setData(ensured);
      } catch(e) {
                        ...
                        <div key={task.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 0", borderBottom: "1px solid #f5f5f5" }}>
                          ...
                          {editingId !== task.id && (
                            <button
                              ...
                              style={{ background: "none", border: "none", cursor: "pointer", color: "#bbb", fontSize: 14, lineHeight: 1, padding: "2px 4px" }}
                              title="수정"
                            >✏️</button>
                          )}
                          <button
                      ...
                      {DAY_KEYS.map((k, i) => {
                        ...
                        return (
                          <tr key={k} style={{ background: i % 2 === 1 ? "#e3f2fd" : "#fff" }}>
                            ...
                            <td style={{ padding: "16px", textAlign: "center", border: "1px solid #e3f2fd", verticalAlign: "top" }}>
                              {row.status ? (
                                <span style={{ display: "inline-block", padding: "4px 10px", borderRadius: 20, fontSize: 12, fontWeight: 700, background: done === total && total > 0 ? "#e8f5e9" : "#e3f2fd", color: done === total && total > 0 ? "#2e7d32" : "#1565c0" }}>
                                  {row.status}
                                </span>
                              ) : "-"}
                            </td>
